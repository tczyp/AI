...
class BrainDQN:
    ...
    def __init__(self, actions):
        ...
        checkpoint = tf.train.get_checkpoint_state("saved_networks_my")
    ...
    def createQNetwork(self):
        ...
        #W_fc1 = self.weight_variable([int(map_width * map_width / 4), 512])
        ...
        stateInput = tf.placeholder("float", [None, map_width, map_width, 4])

        # hidden layers
        temp_conv = self.conv2d(stateInput, W_conv1, 4) + b_conv1
        h_conv1 = tf.nn.relu(temp_conv)
    ...
    def createTrainingMethod(self):
        ...
        self.trainStep = tf.train.AdamOptimizer(LR).minimize(self.cost)
    ...
    def trainQNetwork(self):
        ...
        cost, _ = self.session.run([self.cost, self.trainStep], feed_dict={
            self.yInput: y_batch,
            self.actionInput: action_batch,
            self.stateInput: state_batch
        })

        # print ('cost_sum', np.sum(cost))
        print ('cost:', cost)
        ...
        if self.timeStep % 10000 == 0:
            self.saver.save(self.session, 'saved_networks_my/' + 'network' + '-dqn', global_step=self.timeStep)

        if self.timeStep % UPDATE_TIME == 0:
            self.copyTargetQNetwork()

    def addToReplayMemory(self, replay_item, num):
        for i in range(num):
            self.replayMemory.append(replay_item)
        print ('self.replayMemory.len', len(self.replayMemory))
    ...
    def setPerception(self, nextObservation, action, reward, terminal):
        ...
        # if reward == -1:
        #     self.addToReplayMemory(replay_item, 5)

        while len(self.replayMemory) > REPLAY_MEMORY:
            self.replayMemory.popleft()
        if self.timeStep > OBSERVE and self.no_random:
            # Train the network
            self.trainQNetwork()
    ...
    def getAction(self):
        ...
        if self.timeStep % FRAME_PER_ACTION == 0:
            if random.random() <= self.epsilon and self.no_random == False:
